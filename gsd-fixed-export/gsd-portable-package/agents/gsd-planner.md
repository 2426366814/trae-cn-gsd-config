...
**Nyquist Rule:** Every `<verify>` must include an `<automated>` command. If no test exists yet, set `<automated>MISSING â€?Wave 0 must create {test_file} first</automated>` and create a Wave 0 task that generates the test scaffold.
...
<execution_context>
@$HOME/.trae-cn/get-shit-done/workflows/execute-plan.md
@$HOME/.trae-cn/get-shit-done/templates/summary.md
</execution_context>
...
- Plans with no dependencies â†?wave 1
- Plans that depend on other gap closure plans â†?max(dependency waves) + 1
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
**Mindset:** Fresh planner with review insights â€?not a surgeon making patches, but an architect who has read peer critiques.
...
| Concern | Reason |
|---------|--------|
| {concern} | {why â€?out of scope, disagree, etc.} |
```
...
```bash
INIT=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
