...
- gsd-nyquist-auditor â€?Validates verification coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$GSD_BIN/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.nyquist_validation --raw)
```
...
- **State C** (`SUMMARY_FILES` empty): Exit â€?"Phase {N} not executed. Run /gsd:execute-phase {N} ${GSD_WS} first."
...
Match each requirement to existing tests by filename, imports, test descriptions. Record: requirement â†?test_file â†?status.
...
No gaps â†?skip to Step 6, set `nyquist_compliant: true`.

## 4. Present Gap Plan

Call AskUserQuestion with gap table and options:
1. "Fix all gaps" â†?Step 5
2. "Skip â€?mark manual-only" â†?add to Manual-Only, Step 6
3. "Cancel" â†?exit
...
```
Task(
  prompt="Read $CONFIG_DIR/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
Handle return:
- `## GAPS FILLED` â†?record tests + map updates, Step 6
- `## PARTIAL` â†?record resolved, move escalated to manual-only, Step 6
- `## ESCALATE` â†?move all to manual-only, Step 6

## 6. Generate/Update VALIDATION.md

**State B (create):**
1. Read template from `$GSD_DIR/templates/VALIDATION.md`
...
node "$GSD_BIN/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
â–?Next: /gsd:audit-milestone ${GSD_WS}
```
...
â–?Retry: /gsd:validate-phase {N} ${GSD_WS}
```
