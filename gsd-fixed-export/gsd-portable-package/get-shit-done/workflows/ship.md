...
```bash
CONFIG=$(node "$GSD_BIN/gsd-tools.cjs" state load)
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$GSD_BIN/gsd-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped �?PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} �?PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
