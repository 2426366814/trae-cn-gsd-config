...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
‚Ü?**Skip `spawn_agents` and `collect_confirmations`** ‚Ä?go directly to `sequential_mapping` instead.
...
- Write `.planning/codebase/STACK.md` ‚Ä?Languages, runtime, frameworks, dependencies, configuration
- Write `.planning/codebase/INTEGRATIONS.md` ‚Ä?External APIs, databases, auth providers, webhooks

**Pass 2: Architecture Focus**
- Explore directory structure, entry points, module boundaries, data flow
- Write `.planning/codebase/ARCHITECTURE.md` ‚Ä?Pattern, layers, data flow, abstractions, entry points
- Write `.planning/codebase/STRUCTURE.md` ‚Ä?Directory layout, key locations, naming conventions

**Pass 3: Quality Focus**
- Explore code style, error handling patterns, test files, CI config
- Write `.planning/codebase/CONVENTIONS.md` ‚Ä?Code style, naming, patterns, error handling
- Write `.planning/codebase/TESTING.md` ‚Ä?Framework, structure, mocking, coverage

**Pass 4: Concerns Focus**
- Explore TODOs, known issues, fragile areas, security patterns
- Write `.planning/codebase/CONCERNS.md` ‚Ä?Tech debt, bugs, security, performance, fragile areas
...
```
‚ö†Ô∏è  SECURITY ALERT: Potential secrets detected in codebase documents!
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Initialize project** ‚Ä?use codebase context for planning
