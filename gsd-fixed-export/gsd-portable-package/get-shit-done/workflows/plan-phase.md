...
@$GSD_DIR/references/ui-brand.md
</required_reading>
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
```
...
```bash
PHASE_INFO=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `$GSD_DIR/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "$GSD_BIN/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
- If REQ-ID appears in `PLAN_REQS` �?covered �?- If REQ-ID does NOT appear in any plan �?uncovered �?
