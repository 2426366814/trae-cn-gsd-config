...
// Detect runtime config directory (supports Claude, OpenCode, Gemini, Trae CN)
...
function detectConfigDir(baseDir) {
  ...
  // Priority: Trae CN, then other runtimes
  for (const dir of ['.trae-cn', '.config/opencode', '.opencode', '.gemini', '.claude']) {
    if (fs.existsSync(path.join(baseDir, dir, 'get-shit-done', 'VERSION'))) {
      return path.join(baseDir, dir);
    }
  }
  // Fallback: check for Trae CN directory even without VERSION file
  const traeCnDir = path.join(baseDir, '.trae-cn');
