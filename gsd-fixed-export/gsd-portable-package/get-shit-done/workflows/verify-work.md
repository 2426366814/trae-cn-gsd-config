...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```bash
GSD_BIN=$(node -e "const p=require('path'),f=require('fs'),h=require('os').homedir();const t=p.join(h,'.trae-cn');console.log(p.join(f.existsSync(t)?t:p.join(h,'.claude'),'get-shit-done/bin'))")
CHECKPOINT=$(node "$GSD_BIN/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
```bash
GSD_BIN=$(node -e "const p=require('path'),f=require('fs'),h=require('os').homedir();const t=p.join(h,'.trae-cn');console.log(p.join(f.existsSync(t)?t:p.join(h,'.claude'),'get-shit-done/bin'))")
node "$GSD_BIN/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @$GSD_DIR/workflows/diagnose-issues.md
