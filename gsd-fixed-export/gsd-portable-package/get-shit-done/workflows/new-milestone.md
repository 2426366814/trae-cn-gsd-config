...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init new-milestone)
...
Use template: $GSD_DIR/templates/research-project/{FILE}
</output>
...
**Commit requirements:**
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
