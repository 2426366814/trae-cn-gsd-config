...
3. Update ROADMAP.md ‚Ä?replace milestone details with one-line summary
...
**ROADMAP archive** uses `templates/milestone-archive.md` ‚Ä?includes milestone header (status, phases, date), full phase details, milestone summary (decisions, issues, tech debt).
...
```bash
ROADMAP=$(node "$GSD_BIN/gsd-tools.cjs" roadmap analyze)
```
...
1. **Proceed anyway** ‚Ä?mark milestone complete with known gaps
2. **Run audit first** ‚Ä?`/gsd:audit-milestone` to assess gap severity
3. **Abort** ‚Ä?return to development
...
- Timeline: [Days] days ([Start] ‚Ü?[End])
- Git range: feat(XX-XX) ‚Ü?feat(YY-YY)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$GSD_BIN/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
3. **Requirements audit:**
   ...
   **Validated section:**
   - All Active requirements shipped this milestone ‚Ü?Move to Validated
   - Format: `- ‚ú?[Requirement] ‚Ä?v[X.Y]`
   ...
   **Out of Scope audit:**
   - Review each item ‚Ä?reasoning still valid?
...
5. **Key Decisions audit:**
   ...
   - Mark ‚ú?Good, ‚ö†Ô∏è Revisit, or ‚Ä?Pending
...
**Example full evolution (v1.0 ‚Ü?v1.1 prep):**
...
- Mobile app ‚Ä?web-first approach
- Video chat ‚Ä?use external tools
```
...
- ‚ú?Canvas drawing tools ‚Ä?v1.0
- ‚ú?Real-time sync < 500ms ‚Ä?v1.0 (achieved 200ms avg)
- ‚ú?User authentication ‚Ä?v1.0
...
- Mobile app ‚Ä?web-first approach, PWA works well
- Video chat ‚Ä?use external tools
- Offline mode ‚Ä?real-time is core value
...
Update `.planning/ROADMAP.md` ‚Ä?group completed milestone phases:
...
- ‚ú?**v1.0 MVP** ‚Ä?Phases 1-4 (shipped YYYY-MM-DD)
- üöß **v1.1 Security** ‚Ä?Phases 5-6 (in progress)
- üìã **v2.0 Redesign** ‚Ä?Phases 7-10 (planned)

## Phases

<details>
<summary>‚ú?v1.0 MVP (Phases 1-4) ‚Ä?SHIPPED YYYY-MM-DD</summary>

- [x] Phase 1: Foundation (2/2 plans) ‚Ä?completed YYYY-MM-DD
- [x] Phase 2: Authentication (2/2 plans) ‚Ä?completed YYYY-MM-DD
- [x] Phase 3: Core Features (3/3 plans) ‚Ä?completed YYYY-MM-DD
- [x] Phase 4: Polish (1/1 plan) ‚Ä?completed YYYY-MM-DD
...
```bash
ARCHIVE=$(node "$GSD_BIN/gsd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
Verify: `‚ú?Milestone archived to .planning/milestones/`

**Phase archival (optional):** After archival completes, ask the user:

AskUserQuestion(header="Archive Phases", question="Archive phase directories to milestones/?", options: "Yes ‚Ä?move to milestones/v[X.Y]-phases/" | "Skip ‚Ä?keep phases in place")
...
```
Verify: `‚ú?Phase directories archived to .planning/milestones/v[X.Y]-phases/`
...
**Reorganize ROADMAP.md** ‚Ä?group completed milestone phases:
...
**If doesn't exist:** Create from template at `$GSD_DIR/templates/retrospective.md`.
...
**Commit:**
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init execute-phase "1")
...
Options:
1. **Merge to main** ‚Ä?Merge branch(es) to main
2. **Delete without merging** ‚Ä?Already merged or not needed
3. **Keep branches** ‚Ä?Leave for manual handling
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
```
‚ú?Milestone v[X.Y] [Name] complete
...
**Start Next Milestone** ‚Ä?questioning ‚Ü?research ‚Ü?requirements ‚Ü?roadmap
...
**Version conventions:**
- **v1.0** ‚Ä?Initial MVP
- **v1.1, v1.2** ‚Ä?Minor updates, new features, fixes
- **v2.0, v3.0** ‚Ä?Major rewrites, breaking changes, new direction
...
Heuristic: "Is this deployed/usable/shipped?" If yes ‚Ü?milestone. If no ‚Ü?keep working.
