...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" phase add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs: review backlog â€?promoted N, removed M" --files .planning/ROADMAP.md
   ```

7. **Report summary:**
   ```
   ## ðŸ“‹ Backlog Review Complete
