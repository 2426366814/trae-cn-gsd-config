...
```
Differences:
- Node version: Same âœ?- Environment variables: Same âœ?- Timezone: Different! âœ?
...
**When:** Code constructs paths, URLs, keys, or references from variables â€?and the constructed value might not point where you expect.
...
3. Trace the actual resolved value in both â€?do they agree?
4. Check every variable in the path construction â€?where does each come from? What's its actual value at runtime?
...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = ~/.claude
                  â†?checks $HOME/.trae-cn/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = $HOME/.trae-cn/get-shit-done
                  â†?writes to $HOME/.trae-cn/get-shit-done/hooks/

MISMATCH: Checker looks in wrong directory â†?hooks "not found" â†?reported as stale
```
...
**Important:** A match is a **hypothesis candidate**, not a confirmed diagnosis. Surface it in Current Focus and test it first â€?but do not skip other hypotheses or assume correctness.
...
- If match found:
  - Note in Current Focus: `known_pattern_candidate: "{matched slug} â€?{description}"`
  - Add to Evidence: `found: Knowledge base match on [{keywords}] â†?Root cause was: {root_cause}. Fix was: {fix}.`
  - Test this hypothesis FIRST in Phase 2 â€?but treat it as one hypothesis, not a certainty
...
```bash
INIT=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```markdown
## {slug} â€?{one-line description of the bug}
- **Date:** {ISO date}
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
