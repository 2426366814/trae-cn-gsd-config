...
```bash
PHASE_INFO=$(node "$GSD_BIN/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$GSD_BIN/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
