...
/**
 * Global configuration for Trae IDE paths.
 * Supports both international version (.claude) and Chinese version (.trae-cn).
 * Automatically detects which version is installed based on directory existence.
 */
const TRAE_CONFIG = {
  // Configuration directory name (detected at runtime)
  configDir: null,
  ...
  getConfigDir() {
    ...
    // Priority 1: Chinese version (.trae-cn)
    const traeCnDir = path.join(os.homedir(), '.trae-cn');
    if (fs.existsSync(traeCnDir)) {
      this.configDir = '.trae-cn';
      return this.configDir;
    }

    // Priority 2: International version (.claude)
    ...
    // Default to Chinese version (will be created if needed)
    this.configDir = '.trae-cn';
    return this.configDir;
  },

  // Get full path for a config subdirectory
  getConfigPath(subdir) {
    return path.join(os.homedir(), this.getConfigDir(), subdir);
  },

  // Common paths (computed dynamically)
  get paths() {
    return {
      base: path.join(os.homedir(), this.getConfigDir()),
      agents: this.getConfigPath('agents'),
      commands: this.getConfigPath('commands'),
      skills: this.getConfigPath('skills'),
      projects: this.getConfigPath('projects'),
      gsd: this.getConfigPath('get-shit-done'),
      gsdBin: this.getConfigPath('get-shit-done/bin'),
      userProfile: this.getConfigPath('get-shit-done/USER-PROFILE.md'),
      devPreferences: this.getConfigPath('commands/gsd/dev-preferences.md'),
      claudeMd: this.getConfigPath('CLAUDE.md'),
    };
  },
};

function getGsdDir() {
  return TRAE_CONFIG.paths.gsd;
}

function getGsdBinDir() {
  return TRAE_CONFIG.paths.gsdBin;
}
...
/**
 * Configuration for agents directory resolution.
 * Supports multiple installation modes with priority order.
 */
const AGENTS_DIR_CONFIG = {
  // Priority 1: User directory (standard installation)
  userDir: {
    getPath: () => path.join(os.homedir(), '.trae-cn', 'agents'),
    description: 'User directory (~/.trae-cn/agents)',
  },
  // Priority 2: GSD portable package (portable installation)
  portableDir: {
    getPath: () => path.join(__dirname, '..', '..', '..', 'agents'),
    description: 'GSD portable package',
  },
};

/**
 * Resolve the agents directory with priority order.
 * Checks user directory first, then falls back to GSD portable package.
 * This supports both standard installation and portable mode.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  // Check directories in priority order
  for (const [key, config] of Object.entries(AGENTS_DIR_CONFIG)) {
  ...
  // Default to user directory (will be created if needed)
  return AGENTS_DIR_CONFIG.userDir.getPath();
}
