...
const os = require('os');
const { output, error, safeReadFile, TRAE_CONFIG } = require('./core.cjs');
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = TRAE_CONFIG.paths.userProfile;
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = TRAE_CONFIG.paths.devPreferences;
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = TRAE_CONFIG.paths.claudeMd;
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = TRAE_CONFIG.paths.claudeMd;
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
