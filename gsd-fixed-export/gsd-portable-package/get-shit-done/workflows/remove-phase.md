...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "$GSD_BIN/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "$GSD_BIN/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
- Don't manually renumber â€?use `gsd-tools phase remove` which handles all renumbering
- Don't add "removed phase" notes to STATE.md â€?git commit is the record
