...
@$GSD_DIR/references/git-integration.md
</required_reading>

<available_agent_types>
Valid GSD subagent types (use exact names â€?do not fall back to 'general-purpose'):
- gsd-executor â€?Executes plan tasks, commits, creates SUMMARY.md
</available_agent_types>
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init execute-phase "${PHASE}")
...
**Pattern A:** init_agent_tracking â†?spawn Task(subagent_type="gsd-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash â†?track agent_id â†?wait â†?update tracking â†?report.
...
**If plan contains `<interfaces>` block:** These are pre-extracted type definitions and contracts. Use them directly â€?do NOT re-read the source files to discover types. The planner already extracted what you need.
</step>

<step name="previous_phase_check">
```bash
node "$GSD_BIN/gsd-tools.cjs" phases list --type summaries --raw
...
3. Per task:
   - **MANDATORY read_first gate:** If the task has a `<read_first>` field, you MUST read every listed file BEFORE making any edits. This is not optional. Do not skip files because you "already know" what's in them â€?read them. The read_first files establish ground truth for the task.
...
See `$GSD_DIR/references/tdd.md` for structure.
</tdd_plan_execution>

<precommit_failure_handling>
## Pre-commit Hook Failure Handling

Your commits may trigger pre-commit hooks. Auto-fix hooks handle themselves transparently â€?files get fixed and re-staged automatically.
...
2. Read the error â€?it tells you exactly which hook and what failed
...
```bash
node $GSD_BIN/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
- **Commit it** â€?if it's a source file, config, or intentional artifact
- **Add to .gitignore** â€?if it's a generated/runtime output (build artifacts, `.env` files, cache files, compiled output)
...
See $GSD_DIR/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `false` OR repair returns ESCALATE: STOP. Present: "Verification failed for Task [X]: [name]. Expected: [criteria]. Actual: [result]. Repair attempted: [summary of what was tried]." Options: Retry | Skip (mark incomplete) | Stop (investigate). If skipped â†?SUMMARY "Issues Encountered".
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `$GSD_DIR/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `$GSD_DIR/templates/summary.md`.
...
Next: more plans â†?"Ready for {next-plan}" | last â†?"Phase complete, ready for next step".
</step>
...
node "$GSD_BIN/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$GSD_BIN/gsd-tools.cjs" state update-progress

# Record execution metrics
node "$GSD_BIN/gsd-tools.cjs" state record-metric \
...
node "$GSD_BIN/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$GSD_BIN/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$GSD_BIN/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
