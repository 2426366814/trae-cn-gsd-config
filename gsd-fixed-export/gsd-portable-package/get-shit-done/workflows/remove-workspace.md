...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
```
Cannot remove workspace "$WORKSPACE_NAME" â€?the following repos have uncommitted changes:
...
```
Warning: Could not remove worktree for $REPO_NAME â€?source repo may have been moved or deleted.
```
