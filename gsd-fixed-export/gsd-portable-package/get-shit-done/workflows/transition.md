...
TRANSITION=$(node "$GSD_BIN/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$GSD_BIN/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "$GSD_BIN/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
