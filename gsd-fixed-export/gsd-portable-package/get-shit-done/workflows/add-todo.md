<purpose>
Capture an idea, task, or issue that surfaces during a GSD session as a structured todo for later work. Enables "thought â†?capture â†?continue" flow without losing context.
</purpose>
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init todos)
...
```bash
slug=$(node "$GSD_BIN/gsd-tools.cjs" generate-slug "$title" --raw)
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
