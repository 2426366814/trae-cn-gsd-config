...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init milestone-op)
...
```bash
DETAIL=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "$GSD_BIN/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━�?
 Milestone: {milestone_version} �?{milestone_name}
 Status: Complete �? Lifecycle: audit �?�?complete �?�?cleanup �?
 Ship it! 🚀
```
