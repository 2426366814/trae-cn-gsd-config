...
Resume the thread â€?load its context into the current session:
```bash
...
1. Generate slug from description:
   ```bash
   SLUG=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" generate-slug "$ARGUMENTS")
   ```
...
5. Commit:
   ```bash
   node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs: create thread â€?${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
<notes>
- Threads are NOT phase-scoped â€?they exist independently of the roadmap
- Lighter weight than /gsd:pause-work â€?no phase state, no plan context
- The value is in Context and Next Steps â€?a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /gsd:add-phase or /gsd:add-backlog with context from the thread
- Thread files live in .planning/threads/ â€?no collision with phases or other GSD structures
</notes>
