<purpose>
Extract implementation decisions that downstream agents need ‚Ä?using codebase-first analysis
...
Valid GSD subagent types (use exact names ‚Ä?do not fall back to 'general-purpose'):
- gsd-assumptions-analyzer ‚Ä?Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
without asking the user again. Output is identical to discuss mode ‚Ä?same CONTEXT.md format.
</downstream_awareness>
...
whether your assumptions match their intent ‚Ä?not to answer questions you could figure out
by reading the code.
...
<answer_validation>
**IMPORTANT: Answer validation** ‚Ä?After every AskUserQuestion call, check if the response
...
AGENT_SKILLS_ANALYZER=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-assumptions-analyzer 2>/dev/null)
```
...
**If `--auto`:** Auto-select "Update it". Log: `[auto] Context exists ‚Ä?updating with assumption-based analysis.`

**Otherwise:** Use AskUserQuestion:
- header: "Context"
- question: "Phase [X] already has context. What do you want to do?"
- options:
  - "Update it" ‚Ä?Re-analyze codebase and refresh assumptions
...
**If `--auto`:** Auto-select "Continue and replan after". Log: `[auto] Plans exist ‚Ä?continuing with assumption analysis, will replan after.`
...
- **REQUIREMENTS.md** ‚Ä?Acceptance criteria, constraints
- **STATE.md** ‚Ä?Current progress, any flags
...
**If no prior context exists:** Continue without ‚Ä?expected for early phases.
</step>
...
- conservative OR thorough-evaluator ‚Ü?full_maturity (more alternatives, detailed evidence)
- opinionated ‚Ü?minimal_decisive (fewer alternatives, decisive recommendations)
- pragmatic-fast OR any other value ‚Ü?standard
...
- **Assumption:** [Decision statement]
  - **Why this way:** [Evidence from codebase ‚Ä?cite file paths]
...
[Topics where codebase alone is insufficient ‚Ä?library version compatibility,
...
- `assumptions[]` ‚Ä?each with area, statement, evidence, consequence, confidence
- `needs_research[]` ‚Ä?topics requiring external research (may be empty)
...
```
## Phase {PHASE}: {phase_name} ‚Ä?Assumptions
...
- If all assumptions are Confident or Likely: log assumptions, skip to write_context.
  Log: `[auto] All assumptions Confident/Likely ‚Ä?proceeding to context capture.`
...
- options:
  - "Yes, proceed" ‚Ä?Write CONTEXT.md with these assumptions as decisions
  - "Let me correct some" ‚Ä?Select which assumptions to change
...
- Assumptions ‚Ü?`<decisions>` (each assumption becomes a locked decision: D-01, D-02, etc.)
- Corrections ‚Ü?override the original assumption in `<decisions>`
- Areas where all assumptions were Confident ‚Ü?marked as locked decisions
- Areas with corrections ‚Ü?include user's chosen alternative as the decision
- Folded todos ‚Ü?included in `<decisions>` under "### Folded Todos"
...
{Domain boundary from ROADMAP.md ‚Ä?clear statement of scope anchor}
</domain>
...
### {Area Name 1}
- **D-01:** {Decision ‚Ä?from assumption or correction}
- **D-02:** {Decision}
...
{Accumulated canonical refs from analyze step ‚Ä?full relative paths}
...
{Todos reviewed but not folded ‚Ä?with reason}

[If none: "None ‚Ä?analysis stayed within phase scope"]
</deferred>
```
...
> Decisions captured in CONTEXT.md ‚Ä?this log preserves the analysis.
...
{If no corrections: "No corrections ‚Ä?all assumptions confirmed."}
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
**Phase ${PHASE}: {phase_name}** ‚Ä?{Goal from ROADMAP.md}
...
**Also available:**
- `/gsd:plan-phase ${PHASE} --skip-research` ‚Ä?plan without research
- `/gsd:ui-phase ${PHASE}` ‚Ä?generate UI design contract (if frontend work)
