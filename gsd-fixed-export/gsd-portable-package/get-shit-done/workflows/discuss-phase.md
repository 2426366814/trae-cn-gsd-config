...
You are a thinking partner, not an interviewer. The user is the visionary ‚Ä?you are the builder. Your job is to capture decisions that will guide research and planning, not to figure out implementation yourself.
</purpose>

<downstream_awareness>
**CONTEXT.md feeds into:**

1. **gsd-phase-researcher** ‚Ä?Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" ‚Ü?researcher investigates card component patterns
   - "Infinite scroll decided" ‚Ü?researcher looks into virtualization libraries

2. **gsd-planner** ‚Ä?Reads CONTEXT.md to know WHAT decisions are locked
   - "Pull-to-refresh on mobile" ‚Ü?planner includes that in task specs
   - "Claude's Discretion: loading skeleton" ‚Ü?planner can decide approach
...
```
"[Feature X] would be a new capability ‚Ä?that's its own phase.
...
Gray areas are **implementation decisions the user cares about** ‚Ä?things that could go multiple ways and would change the result.

**How to identify gray areas:**

1. **Read the phase goal** from ROADMAP.md
2. **Understand the domain** ‚Ä?What kind of thing is being built?
   - Something users SEE ‚Ü?visual presentation, interactions, states matter
   - Something users CALL ‚Ü?interface contracts, responses, errors matter
   - Something users RUN ‚Ü?invocation, output, behavior modes matter
   - Something users READ ‚Ü?structure, tone, depth, flow matter
   - Something being ORGANIZED ‚Ü?criteria, grouping, handling exceptions matter
3. **Generate phase-specific gray areas** ‚Ä?Not generic categories, but concrete decisions for THIS phase

**Don't use generic category labels** (UI, UX, Behavior). Generate specific gray areas:

```
Phase: "User authentication"
‚Ü?Session handling, Error responses, Multi-device policy, Recovery flow

Phase: "Organize photo library"
‚Ü?Grouping criteria, Duplicate handling, Naming convention, Folder structure

Phase: "CLI for database backups"
‚Ü?Output format, Flag design, Progress reporting, Error recovery

Phase: "API documentation"
‚Ü?Structure/navigation, Code examples depth, Versioning approach, Interactive elements
```
...
<answer_validation>
**IMPORTANT: Answer validation** ‚Ä?After every AskUserQuestion call, check if the response is empty or whitespace-only. If so:
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "$GSD_BIN/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
**Auto mode** ‚Ä?If `--auto` is present in ARGUMENTS:
...
**If `--auto`:** Auto-select "Update it" ‚Ä?load existing context and continue to analyze_phase. Log: `[auto] Context exists ‚Ä?updating with auto-selected decisions.`

**Otherwise:** Use AskUserQuestion:
- header: "Context"
- question: "Phase [X] already has context. What do you want to do?"
- options:
  - "Update it" ‚Ä?Review and revise existing context
  - "View it" ‚Ä?Show me what's there
  - "Skip" ‚Ä?Use existing context as-is
...
**If `--auto`:** Auto-select "Continue and replan after". Log: `[auto] Plans exist ‚Ä?continuing with context capture, will replan after.`

**Otherwise:** Use AskUserQuestion:
- header: "Plans exist"
- question: "Phase [X] already has {plan_count} plan(s) created without user context. Your decisions here won't affect existing plans unless you replan."
- options:
  - "Continue and replan after" ‚Ä?Capture context, then run /gsd:plan-phase {X} ${GSD_WS} to replan
  - "View existing plans" ‚Ä?Show plans before deciding
  - "Cancel" ‚Ä?Skip discuss-phase
...
Extract from these:
- **PROJECT.md** ‚Ä?Vision, principles, non-negotiables, user preferences
- **REQUIREMENTS.md** ‚Ä?Acceptance criteria, constraints, must-haves vs nice-to-haves
- **STATE.md** ‚Ä?Current progress, any flags or session notes
...
- Read the `<decisions>` section ‚Ä?these are locked preferences
- Read `<specifics>` ‚Ä?particular references or "I want it like X" moments
...
- `discuss_areas`: Pre-fill answers or flag conflicts ("This contradicts Phase 3 ‚Ä?same here or different?")

**If no prior context exists:** Continue without ‚Ä?this is expected for early phases.
</step>
...
```bash
TODO_MATCHES=$(node "$GSD_BIN/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```

Parse JSON for: `todo_count`, `matches[]` (each with `file`, `title`, `area`, `score`, `reasons`).

**If `todo_count` is 0 or `matches` is empty:** Skip silently ‚Ä?no workflow slowdown.
...
```
üìã Found {N} pending todo(s) that may be relevant to Phase {X}:

{For each match:}
- **{title}** (area: {area}, relevance: {score}) ‚Ä?matched on {reasons}
```
...
Lightweight scan of existing code to inform gray area identification and discussion. Uses ~10% context ‚Ä?acceptable for an interactive session.
...
Extract key terms from the phase goal (e.g., "feed" ‚Ü?"post", "card", "list"; "auth" ‚Ü?"login", "session", "token").
...
- **Reusable assets** ‚Ä?existing components, hooks, utilities that could be used in this phase
- **Established patterns** ‚Ä?how the codebase does state management, styling, data fetching
- **Integration points** ‚Ä?where new code would connect (routes, nav, providers)
- **Creative options** ‚Ä?approaches the existing architecture enables or constrains

Store as internal `<codebase_context>` for use in analyze_phase and present_gray_areas. This is NOT written to a file ‚Ä?it's used within this session only.
</step>
...
1. **Domain boundary** ‚Ä?What capability is this phase delivering? State it clearly.

1b. **Initialize canonical refs accumulator** ‚Ä?Start building the `<canonical_refs>` list for CONTEXT.md. This accumulates throughout the entire discussion, not just this step.
   ...
   **Source 4 (discuss_areas):** When the user says "read X", "check Y", or references any doc/spec/ADR during discussion ‚Ä?add it immediately. These are often the MOST important refs because they represent docs the user specifically wants followed.
...
2. **Check prior decisions** ‚Ä?Before generating gray areas, check if any were already decided:
   - Scan `<prior_decisions>` for relevant choices (e.g., "Ctrl+C only, no single-key shortcuts")
   - These are **pre-answered** ‚Ä?don't re-ask unless this phase has conflicting needs
   - Note applicable prior decisions for use in presentation

3. **Gray areas by category** ‚Ä?For each relevant category (UI, UX, Behavior, Empty States, Content), identify 1-2 specific ambiguities that would change implementation. **Annotate with code context where relevant** (e.g., "You already have a Card component" or "No existing pattern for this").

4. **Skip assessment** ‚Ä?If no meaningful gray areas exist (pure infrastructure, clear-cut implementation, or all already decided in prior phases), the phase may not need discussion.
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$GSD_DIR/USER-PROFILE.md"
   ```
   ADVISOR_MODE = file exists at PROFILE_PATH ‚Ü?true, otherwise ‚Ü?false
...
2. If ADVISOR_MODE is true, resolve vendor_philosophy calibration tier:
   ...
   - conservative OR thorough-evaluator ‚Ü?full_maturity
   - opinionated ‚Ü?minimal_decisive
   - pragmatic-fast OR any other value OR empty ‚Ü?standard

3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$GSD_BIN/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```

If ADVISOR_MODE is false, skip all advisor-specific steps ‚Ä?workflow proceeds with existing conversational flow unchanged.
...
Prior decisions: "Minimal UI preferred" (Phase 2), "No pagination ‚Ä?always infinite scroll" (Phase 4)
Gray areas:
- UI: Layout style (cards vs timeline vs grid) ‚Ä?Card component exists with shadow/rounded variants
- UI: Information density (full posts vs previews) ‚Ä?no existing density patterns
- Behavior: Loading pattern ‚Ä?ALREADY DECIDED: infinite scroll (Phase 4)
- Empty State: What shows when no posts exist ‚Ä?EmptyState component exists in ui/
...
```
Phase [X]: [Name]
Domain: [What this phase delivers ‚Ä?from your analysis]
...
- options: Generate 3-4 phase-specific gray areas, each with:
  - "[Specific area]" (label) ‚Ä?concrete, not generic
...
```
‚ò?Exit shortcuts ‚Ä?How should users quit?
  (You decided "Ctrl+C only, no single-key shortcuts" in Phase 5 ‚Ä?revisit or keep?)
```

**Code context annotations:** When the scout found relevant existing code, annotate the gray area description:
```
‚ò?Layout style ‚Ä?Cards vs list vs timeline?
...
```
‚ò?Loading behavior ‚Ä?Infinite scroll or pagination?
  (You chose infinite scroll in Phase 4. useInfiniteQuery hook already set up.)
```

**Do NOT include a "skip" or "you decide" option.** User ran this command to discuss ‚Ä?give them real choices.

**Examples by domain (with code context):**

For "Post Feed" (visual feature):
```
‚ò?Layout style ‚Ä?Cards vs list vs timeline? (Card component exists with variants)
‚ò?Loading behavior ‚Ä?Infinite scroll or pagination? (useInfiniteQuery hook available)
‚ò?Content ordering ‚Ä?Chronological, algorithmic, or user choice?
‚ò?Post metadata ‚Ä?What info per post? Timestamps, reactions, author?
```

For "Database backup CLI" (command-line tool):
```
‚ò?Output format ‚Ä?JSON, table, or plain text? Verbosity levels?
‚ò?Flag design ‚Ä?Short flags, long flags, or both? Required vs optional?
‚ò?Progress reporting ‚Ä?Silent, progress bar, or verbose logging?
‚ò?Error recovery ‚Ä?Fail fast, retry, or prompt for action?
```

For "Organize photo library" (organization task):
```
‚ò?Grouping criteria ‚Ä?By date, location, faces, or events?
‚ò?Duplicate handling ‚Ä?Keep best, keep all, or prompt each time?
‚ò?Naming convention ‚Ä?Original names, dates, or descriptive?
‚ò?Folder structure ‚Ä?Flat, nested by year, or by category?
```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @$CONFIG_DIR/agents/gsd-advisor-researcher.md for your role and instructions.
   ...
   All Task() calls spawn simultaneously ‚Ä?do NOT wait for one before starting the next.
...
3. After ALL agents return, SYNTHESIZE results before presenting:
   ...
   b. Verify all 5 columns present (Option | Pros | Cons | Complexity | Recommendation) ‚Ä?fill any missing columns rather than showing broken table
...
**If ADVISOR_MODE is false:** Skip this step entirely ‚Ä?proceed directly from present_gray_areas to discuss_areas.
</step>
...
Table-first discussion flow ‚Ä?present research-backed comparison tables, then capture user picks.
...
3. **Record the user's selection:**
   - If user picks from table options ‚Ü?record as locked decision for that area
   - If user picks "Other" ‚Ü?receive their input, reflect it back for confirmation, record

4. **After recording pick, Claude decides whether follow-up questions are needed:**
   - If the pick has ambiguity that would affect downstream planning ‚Ü?ask 1-2 targeted follow-up questions using AskUserQuestion
   - If the pick is clear and self-contained ‚Ü?move to next area
   - Do NOT ask the standard 4 questions ‚Ä?the table already provided the context
...
```
"[Feature] sounds like a new capability ‚Ä?that belongs in its own phase.
...
‚Ä?OAuth 2.0 + PKCE is the current standard for SPAs (replaces implicit flow)
‚Ä?Session tokens with httpOnly cookies preferred over localStorage for XSS protection
‚Ä?Consider passkey/WebAuthn support ‚Ä?adoption is accelerating in 2025-2026
...
üí° Recommended: OAuth 2.0 + PKCE ‚Ä?your app has social login in requirements (REQ-04) and this aligns with the existing NextAuth setup in `src/lib/auth.ts`.
...
```
[auto] [Area] ‚Ä?Q: "[question text]" ‚Ü?Selected: "[chosen option]" (recommended default)
```
...
2. **Ask questions using the selected pacing:**
   ...
   **Default (no `--batch`): Ask 4 questions using AskUserQuestion**
   - header: "[Area]" (max 12 chars ‚Ä?abbreviate if needed)
   - question: Specific decision for this area
   - options: 2-3 concrete choices (AskUserQuestion adds "Other" automatically), with the recommended choice highlighted and brief explanation why
   - **Annotate options with code context** when relevant:
     ```
     "How should posts be displayed?"
     - Cards (reuses existing Card component ‚Ä?consistent with Messages)
     - List (simpler, would be a new pattern)
     - Timeline (needs new Timeline component ‚Ä?none exists yet)
     ```
   - Include "You decide" as an option when reasonable ‚Ä?captures Claude discretion
   - **Context7 for library choices:** When a gray area involves library selection (e.g., "magic links" ‚Ü?query next-auth docs) or API approach decisions, use `mcp__context7__*` tools to fetch current documentation and inform the options. Don't use Context7 for every question ‚Ä?only when library-specific knowledge improves the options.
...
3. **After the current set of questions, check:**
   ...
   If "More questions" ‚Ü?ask another 4 single questions, or another 2-5 question batch when `--batch` is active, then check again
   If "Next area" ‚Ü?proceed to next selected area
   If "Other" (free text) ‚Ü?interpret intent: continuation phrases ("chat more", "keep going", "yes", "more") map to "More questions"; advancement phrases ("done", "move on", "next", "skip") map to "Next area". If ambiguous, ask: "Continue with more questions about [area], or move to the next area?"
...
When the user references a doc, spec, or ADR during any answer ‚Ä?e.g., "read adr-014", "check the MCP spec", "per browse-spec.md" ‚Ä?immediately:
...
**Also generate DISCUSSION-LOG.md** ‚Ä?a full audit trail of the discuss-phase Q&A.
...
[Clear statement of what this phase delivers ‚Ä?the scope anchor]
...
### Claude's Discretion
[Areas where user said "you decide" ‚Ä?note that Claude has flexibility here]
...
Every entry needs a full relative path ‚Ä?not just a name.]

### [Topic area 1]
- `path/to/adr-or-spec.md` ‚Ä?[What it decides/defines that's relevant]
- `path/to/doc.md` ¬ßN ‚Ä?[Specific section reference]

### [Topic area 2]
- `path/to/feature-doc.md` ‚Ä?[What this doc defines]

[If no external specs: "No external specs ‚Ä?requirements fully captured in decisions above"]
...
[If none: "No specific requirements ‚Ä?open to standard approaches"]
...
[If none: "None ‚Ä?discussion stayed within phase scope"]
...
## Noted for Later
- [Deferred idea] ‚Ä?future phase

---

## ‚ñ?Next Up

**Phase ${PHASE}: [Name]** ‚Ä?[Goal from ROADMAP.md]

`/gsd:plan-phase ${PHASE} ${GSD_WS}`

<sub>`/clear` first ‚Ü?fresh context window</sub>

---

**Also available:**
- `/gsd:plan-phase ${PHASE} --skip-research ${GSD_WS}` ‚Ä?plan without research
- `/gsd:ui-phase ${PHASE} ${GSD_WS}` ‚Ä?generate UI design contract before planning (if phase has frontend work)
...
> Decisions are captured in CONTEXT.md ‚Ä?this log preserves the alternatives considered.
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" state record-session \
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
2. **Sync chain flag with intent** ‚Ä?if user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ```bash
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node "$GSD_BIN/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read both the chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" config-set workflow._auto_chain_active true
```
...
Display banner:
```
‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚î? GSD ‚ñ?AUTO-ADVANCING TO PLAN
...
Launch plan-phase using the Skill tool to avoid nested Task sessions (which cause runtime freezes due to deep agent nesting ‚Ä?see #686):
```
Skill(skill="gsd:plan-phase", args="${PHASE} --auto ${GSD_WS}")
```

This keeps the auto-advance chain flat ‚Ä?discuss, plan, and execute all run at the same nesting level rather than spawning increasingly deep Task agents.

**Handle plan-phase return:**
- **PHASE COMPLETE** ‚Ü?Full chain succeeded. Display:
  ```
  ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚î?   GSD ‚ñ?PHASE ${PHASE} COMPLETE
  ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚îÅ‚î?
  Auto-advance pipeline finished: discuss ‚Ü?plan ‚Ü?execute

  Next: /gsd:discuss-phase ${NEXT_PHASE} --auto ${GSD_WS}
  <sub>/clear first ‚Ü?fresh context window</sub>
  ```
- **PLANNING COMPLETE** ‚Ü?Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /gsd:execute-phase ${PHASE} ${GSD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** ‚Ü?Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /gsd:plan-phase ${PHASE} ${GSD_WS}
  ```
- **GAPS FOUND** ‚Ü?Stop chain:
  ```
...
Route to `confirm_creation` step (existing behavior ‚Ä?show manual next steps).
</step>
...
- CONTEXT.md includes canonical_refs section with full file paths to every spec/ADR/doc downstream agents need (MANDATORY ‚Ä?never omit)
