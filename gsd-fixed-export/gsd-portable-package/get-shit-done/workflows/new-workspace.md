...
**MANDATORY FIRST STEP â€?Execute init command:**
...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init new-workspace)
...
- `--name` â†?`WORKSPACE_NAME` (required)
- `--repos` â†?`REPO_LIST` (comma-separated paths or names)
- `--path` â†?`TARGET_PATH` (defaults to `$default_workspace_base/$WORKSPACE_NAME`)
- `--strategy` â†?`STRATEGY` (defaults to `worktree`)
- `--branch` â†?`BRANCH_NAME` (defaults to `workspace/$WORKSPACE_NAME`)
- `--auto` â†?skip interactive questions
...
- options:
  - "Yes â€?create workspace with current repo" â†?use current repo
  - "Cancel" â†?exit
...
- options:
  - "Worktree (recommended) â€?lightweight, shares .git objects with source repo" â†?`worktree`
  - "Clone â€?fully independent copy, no connection to source repo" â†?`clone`
...
1. **Target path** â€?must not exist or must be empty:
```bash
...
2. **Source repos exist and are git repos** â€?for each repo path:
```bash
...
3. **Worktree availability** â€?if strategy is `worktree` and `worktree_available` is false:
```
...
- options:
  - "Yes â€?run /gsd:new-project" â†?tell user to `cd $TARGET_PATH` first, then run `/gsd:new-project`
  - "No â€?I'll set it up later" â†?done
