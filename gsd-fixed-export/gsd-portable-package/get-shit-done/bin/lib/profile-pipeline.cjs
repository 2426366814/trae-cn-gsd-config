...
const { output, error, safeReadFile, reapStaleTempFiles, TRAE_CONFIG } = require('./core.cjs');

// ─── Session I/O Helpers ──────────────────────────────────────────────────────

function getSessionsDir(overridePath) {
  const dir = overridePath || TRAE_CONFIG.getConfigPath('projects');
...
async function cmdScanSessions(overridePath, options, raw) {
  ...
  if (!sessionsDir) {
    const searchedPath = overridePath || TRAE_CONFIG.paths.projects;
