...
|---------|----------------|
| `## Decisions` | Locked choices â€?use these as design contract defaults |
| `## Claude's Discretion` | Your freedom areas â€?research and recommend |
| `## Deferred Ideas` | Out of scope â€?ignore completely |

**RESEARCH.md** (if exists) â€?Technical findings from `/gsd:plan-phase`
...
**REQUIREMENTS.md** â€?Project requirements
...
- Confirm 10% accent â€?list the SPECIFIC elements accent is reserved for
...
- `fetch(`, `XMLHttpRequest`, `navigator.sendBeacon` â€?network access
- `process.env` â€?environment variable access
- `eval(`, `Function(`, `new Function` â€?dynamic code execution
...
- **If N or no response:** Do NOT include this block in UI-SPEC.md. Mark registry entry as `BLOCKED â€?developer declined after review`.
- **If Y:** Record in Safety Gate column: `developer-approved after view â€?{date}`

**If NO flags found:**
- Record in Safety Gate column: `view passed â€?no flags â€?{date}`
...
Use template from `$HOME/.trae-cn/get-shit-done/templates/UI-SPEC.md`.

Write to: `$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md`

Fill all sections from the template. For each field:
1. If answered by upstream artifacts â†?pre-populate, note source
2. If answered by user during this session â†?use user's answer
3. If unanswered and has a sensible default â†?use default, note as default
...
- CONTEXT.md â†?locked decisions, discretion areas, deferred ideas
- RESEARCH.md â†?standard stack, architecture patterns
- REQUIREMENTS.md â†?requirement descriptions, success criteria
...
Read template: `$HOME/.trae-cn/get-shit-done/templates/UI-SPEC.md`
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```
