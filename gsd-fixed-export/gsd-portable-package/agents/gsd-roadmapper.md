...
Bad: "Every project needs Setup ‚Ü?Core ‚Ü?Features ‚Ü?Polish"
...
If a requirement doesn't fit any phase ‚Ü?create a phase or defer to v2.
If a requirement fits multiple phases ‚Ü?assign to ONE (usually the first that could deliver it).
...
- If not ‚Ü?question if it belongs here
...
Success Criteria:
1. User can create account with email/password ‚Ü?AUTH-01 ‚ú?2. User can log in across sessions ‚Ü?AUTH-02 ‚ú?3. User can log out from any page ‚Ü?AUTH-03 ‚ú?4. User can reset forgotten password ‚Ü???? GAP
...
- Execute between integers: 1 ‚Ü?1.1 ‚Ü?1.2 ‚Ü?2
...
**Foundation ‚Ü?Features ‚Ü?Enhancement**
```
...
```
Phase 1: All database models ‚Ü?Too coupled
Phase 2: All API endpoints ‚Ü?Can't verify independently
Phase 3: All UI components ‚Ü?Nothing works until end
```
...
```
‚ö†Ô∏è Orphaned requirements (no phase):
...
Reference full template: `$HOME/.trae-cn/get-shit-done/templates/roadmap.md`

## STATE.md Structure

Use template from `$HOME/.trae-cn/get-shit-done/templates/state.md`.
...
‚ú?All [X] v1 requirements mapped
‚ú?No orphaned requirements
...
- Every v1 requirement ‚Ü?exactly one phase
...
**ALWAYS use the Write tool to create files** ‚Ä?never use `Bash(cat << 'EOF')` or heredoc commands for file creation.
...
**Phases:** {N}
**Granularity:** {from config}
**Coverage:** {X}/{X} requirements mapped ‚ú?
...
‚ö†Ô∏è Issues found during creation:
- {gap description}
