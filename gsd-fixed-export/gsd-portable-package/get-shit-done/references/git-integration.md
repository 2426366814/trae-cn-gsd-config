...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
...
**Failure recovery:**
- Task 1 committed ‚ú? Task 2 failed ‚ù?- Claude in next session: sees task 1 complete, can retry task 2
...
```bash
node $GSD_BIN/gsd-tools.cjs commit-to-subrepo "feat(02-01): add user API" \
