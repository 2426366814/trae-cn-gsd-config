...
INIT=$(node "$GSD_BIN/gsd-tools.cjs" init progress)
...
```bash
DISCUSS_MODE=$(node "$GSD_BIN/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
- `STATE=$(node "$GSD_BIN/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$GSD_BIN/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "$GSD_BIN/gsd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node "$GSD_BIN/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```

Parse JSON for `summary.total_items` and `summary.total_files`.

Track: `outstanding_debt` â€?`summary.total_items` from the audit.
...
| {phase} | {filename} | human_needed â€?{count} items |

Review: `/gsd:audit-uat ${GSD_WS}` â€?full cross-phase audit
Resume testing: `/gsd:verify-work {phase} ${GSD_WS}` â€?retest specific phase
```

This is a WARNING, not a blocker â€?routing proceeds normally. The debt is visible so the user can make an informed choice.
...
```bash
PHASE_SECTION=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
**Also available:**
- `/gsd:ui-phase {phase}` â€?generate UI design contract (recommended for frontend phases)
...
`/gsd:discuss-phase {phase} ${GSD_WS}` â€?gather context and clarify approach
...
**Also available:**
- `/gsd:plan-phase {phase} ${GSD_WS}` â€?skip discussion, plan directly
- `/gsd:list-phase-assumptions {phase} ${GSD_WS}` â€?see Claude's assumptions
...
**Also available:**
- `/gsd:execute-phase {phase} ${GSD_WS}` â€?execute phase plans
- `/gsd:verify-work {phase} ${GSD_WS}` â€?run more UAT testing
...
UAT.md exists with `status: partial` â€?testing session ended before all items resolved.
...
`/gsd:verify-work {phase} ${GSD_WS}` â€?resume testing from where you left off
...
**Also available:**
- `/gsd:audit-uat ${GSD_WS}` â€?full cross-phase UAT audit
- `/gsd:execute-phase {phase} ${GSD_WS}` â€?execute phase plans
...
```bash
NEXT_PHASE_SECTION=$(node "$GSD_BIN/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
...
**Also available:**
- `/gsd:ui-phase {Z+1}` â€?generate UI design contract (recommended for frontend phases)
...
`/gsd:discuss-phase {Z+1} ${GSD_WS}` â€?gather context and clarify approach
...
**Also available:**
- `/gsd:plan-phase {Z+1} ${GSD_WS}` â€?skip discussion, plan directly
- `/gsd:verify-work {Z} ${GSD_WS}` â€?user acceptance test before continuing
...
**Also available:**
- `/gsd:verify-work ${GSD_WS}` â€?user acceptance test before completing milestone
...
- Phase complete but next phase not planned â†?offer `/gsd:plan-phase [next] ${GSD_WS}`
- All work complete â†?offer milestone completion
- Blockers present â†?highlight before offering to continue
- Handoff file exists â†?mention it, offer `/gsd:resume-work ${GSD_WS}`
  </step>
