...
Gap: Integration Phase 1�? (Auth not passed to API calls)
...
HIGHEST=$(node "$GSD_BIN/gsd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
