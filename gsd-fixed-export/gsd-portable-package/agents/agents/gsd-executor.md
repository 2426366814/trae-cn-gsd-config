...
```bash
INIT=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
AUTO_CHAIN=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**See @$HOME/.trae-cn/get-shit-done/references/checkpoints.md**
...
- **checkpoint:human-verify** â†?Auto-approve. Log `âš?Auto-approved: [what-built]`. Continue to next task.
...
```bash
node $HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @$HOME/.trae-cn/get-shit-done/templates/summary.md
...
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"

# Add decisions (extract from SUMMARY.md key-decisions)
for decision in "${DECISIONS[@]}"; do
  node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state add-decision \
    --phase "${PHASE}" --summary "${decision}"
done

# Update session info
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state record-session \
  --stopped-at "Completed ${PHASE}-${PLAN}-PLAN.md"
```

```bash
# Update ROADMAP.md progress for this phase (plan counts, status)
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"

# Mark completed requirements from PLAN.md frontmatter
# Extract the `requirements` array from the plan's frontmatter, then mark each complete
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
