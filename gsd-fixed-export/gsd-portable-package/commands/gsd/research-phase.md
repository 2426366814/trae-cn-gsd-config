...
```bash
INIT=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" init phase-op "$ARGUMENTS")
...
```bash
RESEARCHER_MODEL=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node "$HOME/.trae-cn/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}")
```
