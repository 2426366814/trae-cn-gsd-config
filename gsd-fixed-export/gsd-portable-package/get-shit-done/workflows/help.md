...
- Acts as a dispatcher â€?never does the work itself
...
- `--discuss` â€?Lightweight discussion to surface gray areas before planning
- `--research` â€?Focused research agent investigates approaches before planning
- `--full` â€?Adds plan-checking (max 2 iterations) and post-execution verification
...
Execute a trivial task inline â€?no subagents, no planning files, no overhead.

For tasks too small to justify planning: typo fixes, config changes, forgotten commits, simple additions. Runs in the current context, makes the change, commits, and logs to STATE.md.

- No PLAN.md or SUMMARY.md created
- No subagent spawned (runs inline)
- â‰?3 file edits â€?redirects to `/gsd:quick` if task is non-trivial
...
**`/gsd:note <text>`**
Zero-friction idea capture â€?one command, instant save, no questions.

- Saves timestamped note to `.planning/notes/` (or `$CONFIG_DIR/notes/` globally)
...
Cross-AI peer review â€?invoke external AI CLIs to independently review phase plans.
...
- Better than deferred items â€?triggers are checked, not forgotten
...
- `inherit` â€?Use current session model for all agents (OpenCode `/model`)
...
â”œâ”€â”€ RETROSPECTIVE.md      # Living retrospective (updated per milestone)
...
â”œâ”€â”€ milestones/
â”?  â”œâ”€â”€ v1.0-ROADMAP.md       # Archived roadmap snapshot
â”?  â”œâ”€â”€ v1.0-REQUIREMENTS.md  # Archived requirements
â”?  â””â”€â”€ v1.0-phases/          # Archived phase dirs (via /gsd:cleanup or --archive-phases)
