...
- **Project scope**: `.planning/notes/{YYYY-MM-DD}-{slug}.md` â€?used when `.planning/` exists in cwd
- **Global scope**: `$CONFIG_DIR/notes/{YYYY-MM-DD}-{slug}.md` â€?fallback when no `.planning/`, or when `--global` flag is present
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/gsd:note list of groceries` saves a note with text "list of groceries". Same for `promote` â€?only a subcommand when followed by exactly one number.
</step>

<step name="append">
**Subcommand: append â€?create a timestamped note file.**

1. Determine scope (project or global) per storage format above
2. Ensure the notes directory exists (`.planning/notes/` or `$CONFIG_DIR/notes/`)
...
**Constraints:**
- **Never modify the note text** â€?capture verbatim, including typos
- **Never ask questions** â€?just write and confirm
- **Timestamp format**: Use local time, `YYYY-MM-DD HH:mm` (24-hour, no seconds)
</step>

<step name="list">
**Subcommand: list â€?show notes from both scopes.**

1. Glob `.planning/notes/*.md` (if directory exists) â€?project notes
2. Glob `$CONFIG_DIR/notes/*.md` (if directory exists) â€?global notes
...
<step name="promote">
**Subcommand: promote â€?convert a note into a todo.**
...
4. **Requires `.planning/` directory** â€?if it doesn't exist, warn: "Todos require a GSD project. Run `/gsd:new-project` to initialize one."
...
2. **No `.planning/`**: Falls back to global `$CONFIG_DIR/notes/` â€?works in any directory
...
6. **`--global` position**: Stripped from anywhere â€?`--global my idea` and `my idea --global` both save "my idea" globally
...
- [ ] Append: No questions asked â€?instant capture
