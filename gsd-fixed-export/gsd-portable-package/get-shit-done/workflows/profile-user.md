...
Key references:
- @$GSD_DIR/references/ui-brand.md (display patterns)
- @$GSD_DIR/agents/gsd-user-profiler.md (profiler agent definition)
- @$GSD_DIR/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
GSD_DIR=$(node -e "const p=require('path'),f=require('fs'),h=require('os').homedir();const t=p.join(h,'.trae-cn');console.log(p.join(f.existsSync(t)?t:p.join(h,'.claude'),'get-shit-done'))")
...
```bash
cp "$GSD_DIR/USER-PROFILE.md" "$GSD_DIR/USER-PROFILE.backup.md"
  ```
...
�?Stores profile at $GSD_DIR/USER-PROFILE.md
...
SCAN_RESULT=$(node $GSD_BIN/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $GSD_BIN/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `$GSD_DIR/references/user-profiling.md`
...
Reference: @$GSD_DIR/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $GSD_BIN/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $GSD_BIN/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $GSD_BIN/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "? Profile written to $GSD_DIR/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global CLAUDE.md" -- "Add profile to $CONFIG_DIR/CLAUDE.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $GSD_DIR/USER-PROFILE.md" and jump to step 10.
...
```bash
node $GSD_BIN/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "? Generated /gsd:dev-preferences at $CONFIG_DIR/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $GSD_BIN/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $GSD_BIN/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "�? Added profile section to $CONFIG_DIR/CLAUDE.md"
...
```bash
BACKUP_PATH="$GSD_DIR/USER-PROFILE.backup.md"
```
...
Your profile:    $GSD_DIR/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  �?/gsd:dev-preferences   $CONFIG_DIR/commands/gsd/dev-preferences.md
  �?CLAUDE.md section       ./CLAUDE.md
  �?Global CLAUDE.md        $CONFIG_DIR/CLAUDE.md
```
