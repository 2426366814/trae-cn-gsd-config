...
node "$GSD_BIN/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "$GSD_BIN/gsd-tools.cjs" validate health
```
...
if [ -d "$TASKS_DIR" ]; then
  ...
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in $CONFIG_DIR/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf $CONFIG_DIR/tasks/*  (safe �?only affects dead sessions)"
  fi
fi
```
